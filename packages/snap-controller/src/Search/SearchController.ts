...
import { ErrorType, MerchandisingContentBanner, Product } from '@athoscommerce/snap-store-mobx';
...
export class SearchController extends AbstractController {
	...
	search = async (): Promise<void> => {
		try {
			...
			let search: SearchResponseModel & { meta?: MetaResponseModel };
			...
			const data: RenderSchemaData = { responseId: response.search.tracking.responseId };
	...
	public quickview = async ({
		...
		if (!resolvedProductsData && effectiveConfig.fetchProductData !== false) {
			try {
				resolvedProductsData = await this.client.products({ parentId });
			} catch (err) {
		...
		// Even on fetch error, still update so the modal shows partial data (just the
		// cloned result, no variants) and loading=false.
