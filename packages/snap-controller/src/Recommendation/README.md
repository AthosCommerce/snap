...
The Recommendation controller exposes a `quickview` method for opening the product quickview modal. The modal state does not live on `RecommendationController` — it is owned by the `QuickviewManager` (see `snap-controller`), reachable at `window.athos.quickview` / `snap.quickview`, whose `store` is a `QuickviewStore`. A single quickview component is rendered once: the Snap framework injects it into `<body>`, so consumers no longer render one per result.
