...
- `variantSelections` — one `VariantSelection` per variant selection. When `inline` (the chat panel), the variant title includes the value count (e.g. "Color (5)") and non-swatch selections render as a `list` of selectable tiles instead of a dropdown.
...
| `inline` | `boolean` | | Embedded in another panel (e.g. the chat secondary window via `ChatProductQueryMessage`): no `role="dialog"`/`aria-modal` focus trap, no window-Escape close, and no built-in close button — the host panel owns dismissal. Also switches modules to the chat presentation: variant titles include the value count, non-swatch selections render as tile lists, and the add-to-cart/similar/discuss buttons carry icons. |
