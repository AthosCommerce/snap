...
export default {
	...
	argTypes: {
		...
		thumbnailSwatches: {
			description:
				"use each variant's thumbnailImageUrl as the swatch image when the value has no background of its own (swatches only, and only when the thumbnails differ)",
			table: {
				category: 'Templates Legal',
				type: {
					summary: 'boolean',
...
export const SwatchesWithThumbnails = (args: VariantSelectionProps) => {
	return <ObservableSelection args={args} />;
};
...
SwatchesWithThumbnails.args = {
	selection: {
		...
		field: 'color',
		label: 'color',
		selected: { value: 'Indigo', label: 'Indigo', available: true },
		values: thumbnailValues,
	},
	type: 'swatches',
