...
export const Pagination = observer((properties: PaginationProps) => {
	...
	const { overrideElement, shouldRenderDefault } = useCustomComponentOverride('pagination', props);
				...
				<nav role="navigation" aria-label="Pagination">
					...
					{(!pageNumbers.includes(store.first.number) || (persistFirst && store.page !== store.first.number)) && !hideFirst && (
						<>
							...
							{!pageNumbers.includes(2) && !hideEllipsis && <span>{ellipsisButton ? ellipsisButton : <>&hellip;</>}</span>}
						</>
					)}
					...
					{(!pageNumbers.includes(store.last.number) || (persistLast && store.page !== store.last.number)) && !hideLast && (
						<>
							{!pageNumbers.includes(store.totalPages - 1) && !hideEllipsis && <span>{ellipsisButton ? ellipsisButton : <>&hellip;</>}</span>}
