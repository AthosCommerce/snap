	...
	(self.webpackChunk_athoscommerce_snap_preact = self.webpackChunk_athoscommerce_snap_preact || []).push([
		[6225],
		{
			'./components/src/components/Templates/RecommendationBundle/RecommendationBundle.stories.tsx'(v, r, e) {
				'use strict';
				e.r(r), e.d(r, { Default: () => c, __namedExportsOrder: () => d, default: () => y });
...
`;
				...
				const y = {
					...
					B = { id: 'RecommendationBundle', tag: 'bundley', globals: { siteId: 'atkzs2', products: ['VW1982-UQZ-AS'] } },
					C = i.p.recommendation(B),
					c = (f, { loaded: { controller: x } }) => (0, o.Y)(n.g, { ...f, controller: x, results: x.store.results });
				(c.loaders = [
					async () => (
						C.on('afterStore', async ({ controller: f }, x) => {
  ...
  return <RecommendationBundle {...props} controller={controller} results={controller.store.results} />;
					...
					return (
						(0, o.vJ)(() => {
							const p = new IntersectionObserver(
								([y]) => {
									_(y.isIntersecting), t && y.isIntersecting && p.unobserve(n.current);
								},
								{ rootMargin: b }
							);
							return (
					...
					n = (t, i = {}) => {
						...
						return (
							(0, o.vJ)(() => {
								...
								const I = () => {
										w && (window.clearInterval(w), (w = null));
									},
									A = () => {
										B > 0
											? ((f.current = Date.now()),
											  ...
											  (d.current = window.setTimeout(() => {
													c(!0), p && t.current && g && g.unobserve(t.current);
											  }, B)))
											: (c(!0), p && t.current && g && g.unobserve(t.current));
									},
									s = () => {
										d.current && window.clearTimeout(d.current), (d.current = null), (f.current = null), c(!1);
									};
								return (
									...
									() => {
										c(!1), I(), d.current && window.clearTimeout(d.current), g && t.current && g.unobserve(t.current);
									}
								);
							}, [t, x]),
			...
			'./components/src/utilities/createImpressionObserver.ts'(v, r, e) {
				...
				function t(i) {
					const _ = (0, o.li)(null),
						{ inViewport: p, updateRef: y } = (0, a.v)(_, { ...i, fireOnce: !0, threshold: n, minVisibleTime: b });
			...
			'./components/src/utilities/snapify.ts'(v, r, e) {
				...
				class g {
					static recommendation(l) {
						const m = l.id;
						if (h[m]) return h[m];
						const T = (h[m] = I({ client: R, controller: l }));
						return (
							T.on('afterStore', async ({ controller: u }, E) => {
					...
					static autocomplete(l) {
						const m = l.id;
						if (h[m]) return h[m];
						const T = (h[m] = A({ client: R, controller: l }));
						return (
							T.on('afterStore', async ({ controller: u }, E) => {
								u.log.debug('controller', u), u.log.debug('store', u.store.toJSON()), await E();
							}),
					...
					static search(l) {
						const m = l.id;
						if (h[m]) return h[m];
						const T = (h[m] = w({ client: R, controller: l }));
						return (
							T.on('afterStore', async ({ controller: u }, E) => {
								u.log.debug('controller', u), u.log.debug('store', u.store.toJSON()), await E();
							}),
				...
				function w(s) {
					const l = new y.V(new C.E({ settings: { coreType: 'query', corePrefix: s.controller.id } }), B.X);
				...
				function I(s) {
					const l = new y.V(new C.E(), B.X).detach(!0);
				...
				function A(s) {
					const l = new y.V(new C.E(), B.X).detach();
...
//# sourceMappingURL=components-Templates-RecommendationBundle-RecommendationBundle-stories.671e4712.iframe.bundle.js.map
