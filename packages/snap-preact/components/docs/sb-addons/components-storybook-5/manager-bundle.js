try {
	(() => {
		var l = __REACT__,
			{
				...
				StrictMode: m2,
				Suspense: b2,
				__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED: y2,
				cloneElement: L2,
				createContext: x2,
				createElement: v2,
				createFactory: S2,
		...
		var N2 = __STORYBOOK_API__,
			{
				...
				Provider: Y2,
				addons: B,
				combineParameters: W2,
				controlOrMetaKey: X2,
				controlOrMetaSymbol: J2,
				eventMatchesShortcut: Q2,
				eventToShortcut: V2,
				isMacLike: e1,
				isShortcutTaken: t1,
				keyToSymbol: r1,
				merge: a1,
				mockChannel: n1,
				optionOrAltSymbol: o1,
				shortcutMatchesShortcut: i1,
				shortcutToHumanString: s1,
				types: e9,
				useAddonState: l1,
				useArgTypes: p1,
				useArgs: d1,
		...
		var F1 = __STORYBOOK_CLIENT_LOGGER__,
			{ deprecate: _1, logger: a9, once: M1, pretty: E1 } = __STORYBOOK_CLIENT_LOGGER__;
		function k() {
			return (
				(k = Object.assign
					? Object.assign.bind()
		...
		function X(e) {
			...
			return (
				(X = function (r) {
					...
					function a() {
						return D(r, arguments, W(this).constructor);
					}
...
`,
		...
		function v9() {
			...
			var a = t[0],
				n = [],
				i;
			for (i = 1; i < t.length; i += 1) n.push(t[i]);
			return (
		...
		var x = (function (e) {
			b9(t, e);
			function t(r) {
				for (var a, n = arguments.length, i = new Array(n > 1 ? n - 1 : 0), s = 1; s < n; s++) i[s - 1] = arguments[s];
				return (a = e.call(this, v9.apply(void 0, [x9[r]].concat(i))) || this), m9(a);
			}
			return t;
		})(X(Error));
		function N(e) {
		...
		function O(e, t, r, a) {
			if ((a === void 0 && (a = S9), t === 0)) return a(r, r, r);
			var n = (((e % 360) + 360) % 360) / 60,
				i = (1 - Math.abs(2 * r - 1)) * t,
				s = i * (1 - Math.abs((n % 2) - 1)),
				f = 0,
				u = 0,
				c = 0;
			n >= 0 && n < 1
		...
		var k9 = /^#[a-fA-F0-9]{6}$/,
			...
			_9 = /^#[a-fA-F0-9]{3}$/,
			M9 = /^#[a-fA-F0-9]{4}$/,
			q = /^rgb\(\s*(\d{1,3})\s*(?:,)?\s*(\d{1,3})\s*(?:,)?\s*(\d{1,3})\s*\)$/i,
			E9 = /^rgb(?:a)?\(\s*(\d{1,3})\s*(?:,)?\s*(\d{1,3})\s*(?:,)?\s*(\d{1,3})\s*(?:,|\/)\s*([-+]?\d*[.]?\d+[%]?)\s*\)$/i,
			O9 = /^hsl\(\s*(\d{0,3}[.]?[0-9]+(?:deg)?)\s*(?:,)?\s*(\d{1,3}[.]?[0-9]?)%\s*(?:,)?\s*(\d{1,3}[.]?[0-9]?)%\s*\)$/i,
			P9 =
				/^hsl(?:a)?\(\s*(\d{0,3}[.]?[0-9]+(?:deg)?)\s*(?:,)?\s*(\d{1,3}[.]?[0-9]?)%\s*(?:,)?\s*(\d{1,3}[.]?[0-9]?)%\s*(?:,|\/)\s*([-+]?\d*[.]?\d+[%]?)\s*\)$/i;
		function $(e) {
			if (typeof e != 'string') throw new x(3);
			var t = w9(e);
			if (t.match(k9)) return { red: parseInt('' + t[1] + t[2], 16), green: parseInt('' + t[3] + t[4], 16), blue: parseInt('' + t[5] + t[6], 16) };
			if (t.match(F9)) {
				var r = parseFloat((parseInt('' + t[7] + t[8], 16) / 255).toFixed(2));
				return { red: parseInt('' + t[1] + t[2], 16), green: parseInt('' + t[3] + t[4], 16), blue: parseInt('' + t[5] + t[6], 16), alpha: r };
			}
			if (t.match(_9)) return { red: parseInt('' + t[1] + t[1], 16), green: parseInt('' + t[2] + t[2], 16), blue: parseInt('' + t[3] + t[3], 16) };
			if (t.match(M9)) {
			...
			var i = E9.exec(t.substring(0, 50));
			if (i)
				return {
					red: parseInt('' + i[1], 10),
			...
			var s = O9.exec(t);
			if (s) {
				var f = parseInt('' + s[1], 10),
					u = parseInt('' + s[2], 10) / 100,
					c = parseInt('' + s[3], 10) / 100,
					g = 'rgb(' + O(f, u, c) + ')',
					m = q.exec(g);
				if (!m) throw new x(4, t, g);
				return { red: parseInt('' + m[1], 10), green: parseInt('' + m[2], 10), blue: parseInt('' + m[3], 10) };
			}
			var h = P9.exec(t.substring(0, 50));
			if (h) {
				var F = parseInt('' + h[1], 10),
					T = parseInt('' + h[2], 10) / 100,
					_ = parseInt('' + h[3], 10) / 100,
					I = 'rgb(' + O(F, T, _) + ')',
					w = q.exec(I);
				if (!w) throw new x(4, t, I);
				return {
					red: parseInt('' + w[1], 10),
		...
		function R9(e) {
			var t = e.red / 255,
				...
				i = Math.min(t, r, a),
				s = (n + i) / 2;
			if (n === i) return e.alpha !== void 0 ? { hue: 0, saturation: 0, lightness: s, alpha: e.alpha } : { hue: 0, saturation: 0, lightness: s };
			var f,
				u = n - i,
				c = s > 0.5 ? u / (2 - n - i) : u / (n + i);
			switch (n) {
				case t:
					f = (r - a) / u + (r < a ? 6 : 0);
					break;
			...
			return (f *= 60), e.alpha !== void 0 ? { hue: f, saturation: c, lightness: s, alpha: e.alpha } : { hue: f, saturation: c, lightness: s };
		}
		function s9(e) {
			return R9($(e));
		}
		...
		function I9(e, t, r) {
			return J('#' + U(e) + U(t) + U(r));
		}
		function z(e, t, r) {
			return O(e, t, r, I9);
		}
		function Z9(e, t, r) {
			if (typeof e == 'number' && typeof t == 'number' && typeof r == 'number') return z(e, t, r);
			if (typeof e == 'object' && t === void 0 && r === void 0) return z(e.hue, e.saturation, e.lightness);
			throw new x(1);
		}
		function B9(e, t, r, a) {
			if (typeof e == 'number' && typeof t == 'number' && typeof r == 'number' && typeof a == 'number')
				return a >= 1 ? z(e, t, r) : 'rgba(' + O(e, t, r) + ',' + a + ')';
			if (typeof e == 'object' && t === void 0 && r === void 0 && a === void 0)
				return e.alpha >= 1 ? z(e.hue, e.saturation, e.lightness) : 'rgba(' + O(e.hue, e.saturation, e.lightness) + ',' + e.alpha + ')';
			throw new x(2);
		}
		function Q(e, t, r) {
			if (typeof e == 'number' && typeof t == 'number' && typeof r == 'number') return J('#' + v(e) + v(t) + v(r));
			if (typeof e == 'object' && t === void 0 && r === void 0) return J('#' + v(e.red) + v(e.green) + v(e.blue));
			throw new x(6);
		}
		function P(e, t, r, a) {
			if (typeof e == 'string' && typeof t == 'number') {
				...
				if (typeof e == 'object' && t === void 0 && r === void 0 && a === void 0)
					return e.alpha >= 1 ? Q(e.red, e.green, e.blue) : 'rgba(' + e.red + ',' + e.green + ',' + e.blue + ',' + e.alpha + ')';
			}
			throw new x(7);
		}
		var D9 = function (e) {
		...
		function p9(e, t, r) {
			return function () {
				...
				return a.length >= t ? e.apply(this, a) : p9(e, t, a);
			};
		}
		function H(e) {
		...
		function $9(e, t) {
			...
			var r = s9(t);
			return l9(k({}, r, { lightness: G(0, 1, r.lightness - parseFloat(e)) }));
		}
		var H9 = H($9),
			G9 = H9;
		function N9(e, t) {
		...
		function K9(e, t) {
			...
			var r = $(t),
				...
				n = k({}, r, { alpha: G(0, 1, (a * 100 + parseFloat(e) * 100) / 100) });
			return P(n);
		}
		var B1 = H(K9);
		function Y9(e, t) {
			...
			var r = $(t),
				a = typeof r.alpha == 'number' ? r.alpha : 1,
				n = k({}, r, { alpha: G(0, 1, +(a * 100 - parseFloat(e) * 100).toFixed(2) / 100) });
			return P(n);
		}
		var W9 = H(Y9),
			X9 = W9,
			...
			J9 = {
				...
				appBg: o9.app,
				appContentBg: p.lightest,
				appPreviewBg: p.lightest,
				appBorderColor: p.border,
				appBorderRadius: 4,
				fontBase: A.fonts.base,
				fontCode: A.fonts.mono,
				textColor: p.darkest,
				textInverseColor: p.lightest,
				textMutedColor: p.dark,
				barTextColor: p.mediumdark,
				barHoverColor: p.secondary,
				barSelectedColor: p.secondary,
				barBg: p.lightest,
				buttonBg: o9.app,
				buttonBorder: p.medium,
				booleanBg: p.mediumlight,
				booleanSelectedBg: p.lightest,
				inputBg: p.lightest,
				inputBorder: p.border,
				inputTextColor: p.darkest,
				inputBorderRadius: 4,
			},
			i9 = J9,
			Q9 = {
				...
				inputTextColor: p.lightest,
				inputBorderRadius: 4,
			},
			V9 = Q9,
			{ window: K } = r9;
		var e2 = (e) =>
				typeof e != 'string' ? (a9.warn(`Color passed to theme object should be a string. Instead ${e}(${typeof e}) was passed.`), !1) : !0,
			t2 = (e) => !/(gradient|var|calc)/.test(e),
			r2 = (e, t) => (e === 'darken' ? P(`${G9(1, t)}`, 0.95) : e === 'lighten' ? P(`${U9(1, t)}`, 0.95) : t),
			d9 = (e) => (t) => {
				if (!e2(t) || !t2(t)) return t;
				try {
					return r2(e, t);
				} catch {
					return t;
		...
		var c9 = __REACT_DOM__,
			{
				__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED: t0,
				createPortal: r0,
				findDOMNode: a0,
				flushSync: n0,
				hydrate: o0,
				render: i0,
				unmountComponentAtNode: s0,
				unstable_batchedUpdates: l0,
				unstable_createPortal: p0,
		...
		var S = ['primary', 'secondary', 'accent'],
			...
			h9 = () => {
				let [e, t] = t9(),
					...
					h = () => {
						if (s.current) {
							let o = s.current.getBoundingClientRect();
							u({ top: o.bottom + 14, left: o.left });
						}
						a(!0);
					};
				l.useEffect(() => {
					if (!r) return;
					let o = (C) => {
						let M = document.getElementById('ss-color-picker-panel');
						s.current?.contains(C.target) || M?.contains(C.target) || a(!1);
					};
					document.addEventListener('mousedown', o);
					let d = document.querySelector('#storybook-preview-iframe');
					return (
						d?.contentDocument?.addEventListener('mousedown', o),
						() => {
							document.removeEventListener('mousedown', o), d?.contentDocument?.removeEventListener('mousedown', o);
						}
					);
				}, [r]),
					l.useEffect(
				...
				let F = (o, d) => {
						g((C) => ({ ...C, [o]: d })),
							clearTimeout(m.current[o]),
							(m.current[o] = setTimeout(() => {
								t({ [`${R}${o}`]: d });
							}, o2));
					},
					T = (o) => {
						clearTimeout(m.current[o]), g((d) => ({ ...d, [o]: '' })), t({ [`${R}${o}`]: '' });
					},
					_ = S.some((o) => c[o]),
					I = S.map((o) => {
						let d = c[o],
					...
					w = r
						? c9.createPortal(
								l.createElement(
									...
									l.createElement(
										'div',
										{ style: { fontSize: '11px', fontWeight: 600, color: '#666', marginBottom: '8px' } },
									...
									...S.map((o) => {
										...
										return l.createElement(
											'div',
											{ key: o, style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 0' } },
											l.createElement(
												'div',
												{
													style: {
														...
														height: '24px',
														borderRadius: '3px',
														border: `2px solid ${Z ? '#1D4990' : 'transparent'}`,
														background: M,
														flexShrink: 0,
														overflow: 'hidden',
														cursor: 'pointer',
												...
												l.createElement('input', {
													type: 'color',
													value: M,
													onChange: (g9) => F(o, g9.target.value),
													style: {
														position: 'absolute',
														inset: 0,
														width: '200%',
											...
											l.createElement(
												...
												l.createElement('span', { style: { fontSize: '9px', color: '#aaa', fontFamily: 'monospace', lineHeight: '1.2' } }, M)
											),
											Z
												? l.createElement(
														'button',
												...
												: l.createElement('div', { style: { width: '22px', flexShrink: 0 } })
										);
									}),
									_
										? l.createElement(
					...
					l.createElement(
						'button',
						{
							...
							style: {
								...
								border: 'none',
								borderRadius: '4px',
								background: r || n ? 'rgba(0,0,0,0.12)' : 'rgba(0,0,0,0.06)',
								color: _ ? '#1D4990' : '#444',
								fontSize: '13px',
								fontWeight: 700,
								whiteSpace: 'nowrap',
						...
						l.createElement(
							...
							...I.map((o, d) =>
								l.createElement('div', {
									key: d,
									style: { width: '9px', height: '9px', borderRadius: '50%', background: o, border: '1px solid rgba(0,0,0,0.18)', flexShrink: 0 },
								})
							)
						),
						l.createElement('span', { style: { letterSpacing: '0.01em' } }, 'Colors'),
						_
							? l.createElement('span', {
									title: 'Colors overridden',
									style: { width: '6px', height: '6px', borderRadius: '50%', background: '#1D4990', flexShrink: 0, marginLeft: '1px' },
							  })
							: null
					),
					w
		...
		B.setConfig({ theme: u9 });
		B.register('snap-theme-color-picker', () => {
			B.add('snap-theme-color-picker/tool', { type: e9.TOOL, title: 'Theme Colors', render: () => l.createElement(h9) });
		});
	})();
} catch (e) {
