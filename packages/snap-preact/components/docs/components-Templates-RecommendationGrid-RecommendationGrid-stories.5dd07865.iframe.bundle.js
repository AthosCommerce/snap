	...
	(self.webpackChunk_athoscommerce_snap_preact = self.webpackChunk_athoscommerce_snap_preact || []).push([
		[401],
		{
			'./components/src/components/Templates/RecommendationGrid/RecommendationGrid.stories.tsx'(I, c, e) {
				'use strict';
				e.r(c), e.d(c, { Grid: () => f, List: () => h, __namedExportsOrder: () => O, default: () => l });
  ...
  return <div style={{
    maxWidth: '1200px'
			...
			'./components/src/components/Templates/RecommendationGrid/RecommendationGrid.tsx'(I, c, e) {
				...
				const i = ({ gapSize: _, columns: v }) =>
					...
					u = (0, p.PA)((_) => {
						...
						let m = (0, f.v6)('recommendationGrid', v, A, k);
						if (!(_.theme?.type == 'templates' || v?.type == 'templates')) {
			...
			'./components/src/components/Trackers/Recommendation/ProfileTracker/RecommendationProfileTracker.tsx'(I, c, e) {
				...
				const l = () => (0, a.AH)({}),
					E = (0, n.PA)((h) => {
						const { children: f, className: O, internalClassName: d } = h,
							P = (0, p.v2)(f),
							x = (0, r.Z)(h, l);
						return P.length ? (0, s.Y)('div', { className: o()('ss__recommendation-profile-tracker', O, d), ...x, children: f }) : null;
					});
			},
										...
										h > 0
											? ((P.current = Date.now()),
											  ...
											  (d.current = window.setTimeout(() => {
													O(!0), l && t.current && y && y.unobserve(t.current);
											  }, h)))
											: (O(!0), l && t.current && y && y.unobserve(t.current));
									},
									i = () => {
										d.current && window.clearTimeout(d.current), (d.current = null), (P.current = null), O(!1);
									};
								return (
									...
									() => {
										O(!1), C(), d.current && window.clearTimeout(d.current), y && t.current && y.unobserve(t.current);
									}
								);
							}, [t, x]),
			...
			'./components/src/utilities/snapify.ts'(I, c, e) {
				...
				class y {
					static recommendation(u) {
						const _ = u.id;
						if (R[_]) return R[_];
						const v = (R[_] = C({ client: L, controller: u }));
						return (
							v.on('afterStore', async ({ controller: g }, A) => {
					...
					static autocomplete(u) {
						const _ = u.id;
						if (R[_]) return R[_];
						const v = (R[_] = b({ client: L, controller: u }));
						return (
							v.on('afterStore', async ({ controller: g }, A) => {
								g.log.debug('controller', g), g.log.debug('store', g.store.toJSON()), await A();
							}),
					...
					static search(u) {
						const _ = u.id;
						if (R[_]) return R[_];
						const v = (R[_] = T({ client: L, controller: u }));
						return (
							v.on('afterStore', async ({ controller: g }, A) => {
								g.log.debug('controller', g), g.log.debug('store', g.store.toJSON()), await A();
							}),
...
//# sourceMappingURL=components-Templates-RecommendationGrid-RecommendationGrid-stories.5dd07865.iframe.bundle.js.map
