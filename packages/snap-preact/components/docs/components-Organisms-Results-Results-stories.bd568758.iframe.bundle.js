						...
						(0, m.W)(
							(0, g.PA)((v) => {
								const M = (0, r.u)(),
									...
									{ overrideElement: W, shouldRenderDefault: V } = (0, O._)('inlineBanner', d);
								if (!V) return W;
								const J = (0, p.Z)(d, P);
								return _ && _.value
									? (0, s.Y)(t._, {
				...
				const y = ({ gapSize: B, columns: W }) =>
					...
					U = (0, n.PA)((B) => {
						...
						const { disableStyles: H, className: q, internalClassName: ee, layout: te, theme: F, excludeBanners: se, controller: K, treePath: G } = h,
							Y = h.resultComponent,
							{ overrideElement: ne, shouldRenderDefault: z } = (0, D._)('results', h);
						if (!z) return ne;
						...
						let $ = se ? h.results?.filter((L) => L.type !== m.c.BANNER) : h.results;
						h?.columns && h?.rows && h.columns > 0 && h.rows > 0 && ($ = $?.slice(0, h.columns * h.rows));
...
//# sourceMappingURL=components-Organisms-Results-Results-stories.bd568758.iframe.bundle.js.map
