...
`,
  ...
  return <RecommendationEmail {...props} controller={controller} />;
				...
				const k = () =>
					...
					j = (0, a.PA)((G) => {
						...
						const v = h?.display?.mappings.core || h?.mappings?.core,
							...
							T = {
								variantSelection: {
									...
									...(0, d.s)({ disableStyles: U }),
									theme: f.theme,
									treePath: O,
								},
								price: { internalClassName: 'ss__result__price', ...(0, d.s)({ disableStyles: U }), theme: f.theme, treePath: O },
								calloutBadge: {
								...
								image: {
									...
									alt: v?.name || '',
									src: v?.imageUrl || '',
									...(0, d.s)({ disableStyles: U, fallback: X }),
									theme: f.theme,
									treePath: O,
								},
								rating: {
						...
						const _e = (0, P.Z)(f, k),
							...
							ve = u()(he, f.lang || {}),
							oe = (0, p.u)(ve, { result: h, controller: W }, { activeBreakpoint: V?.activeBreakpoint }),
									...
									children: (0, s.FD)('article', {
										...
										children: [
											...
											(0, s.FD)('div', {
												...
												children: [
													!K && (0, s.Y)(L.W, { ...T.calloutBadge, controller: W }),
													!F &&
														(0, s.Y)('div', {
			...
			'./components/src/components/Templates/RecommendationEmail/RecommendationEmail.tsx'(x, i, e) {
				...
				const M = { default: {}, mobile: {}, tablet: {}, desktop: {} },
					L = () => (0, r.AH)({}),
					w = (0, a.PA)((D) => {
						const R = (0, d.u)(),
							m = (0, P.LU)(),
							...
							{ controller: E, results: _, resultWidth: k, treePath: j, disableStyles: G, internalClassName: V, className: J } = p,
							S = p.resultComponent,
							f = (0, A.uk)(),
							h = typeof S == 'string',
							K = h ? S : '',
							F = f?.templates?.library.import.component.result || {},
							{ ComponentOverride: H, shouldWaitForNamedOverride: $ } = (0, C.x)(F, h ? K : void 0),
							N = h ? H : S,
							X = {
									...
									children: Y.map((z, Q) =>
										(0, s.Y)(
											'div',
											{
												...
												style: { display: 'block', width: k },
												children: N
													? (0, b.Y)(N, {
															controller: E,
															result: z,
															email: !0,
															theme: h ? t()(p.theme || {}, { components: { result: { customComponent: S }, image: { lazy: !1 } } }) : p.theme,
															treePath: j,
													  })
													: $
													? null
			...
			'./components/src/utilities/snapify.ts'(x, i, e) {
				...
				class L {
					static recommendation(u) {
						const p = u.id;
						if (C[p]) return C[p];
						const E = (C[p] = D({ client: M, controller: u }));
						return (
							E.on('afterStore', async ({ controller: _ }, k) => {
								_.log.debug('controller', _), _.log.debug('store', _.store.toJSON()), await k();
							}),
							E.init(),
							E
						);
					...
					static autocomplete(u) {
						const p = u.id;
						if (C[p]) return C[p];
						const E = (C[p] = R({ client: M, controller: u }));
						return (
							E.on('afterStore', async ({ controller: _ }, k) => {
								_.log.debug('controller', _), _.log.debug('store', _.store.toJSON()), await k();
							}),
					...
					static search(u) {
						const p = u.id;
						if (C[p]) return C[p];
						const E = (C[p] = w({ client: M, controller: u }));
						return (
							E.on('afterStore', async ({ controller: _ }, k) => {
								_.log.debug('controller', _), _.log.debug('store', _.store.toJSON()), await k();
							}),
				...
				var s = function (r, a) {
					...
					(r.prototype.toName = function (c) {
						...
						if (c?.closest) {
							...
							for (var C in o) {
								var M = ((b = I), (g = n[C]), Math.pow(b.r - g.r, 2) + Math.pow(b.g - g.g, 2) + Math.pow(b.b - g.b, 2));
								M < d && ((d = M), (A = C));
							}
							return A;
						}
...
//# sourceMappingURL=components-Templates-RecommendationEmail-RecommendationEmail-stories.0ceac81d.iframe.bundle.js.map
