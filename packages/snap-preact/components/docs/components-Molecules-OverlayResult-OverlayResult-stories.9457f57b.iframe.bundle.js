(() => {
	(self.webpackChunk_athoscommerce_snap_preact = self.webpackChunk_athoscommerce_snap_preact || []).push([
		[1663],
		{
			...
			'./components/src/components/Molecules/OverlayResult/OverlayResult.stories.tsx'(O, i, e) {
				'use strict';
...
`,
  ...
  return <div style={{
    maxWidth: '300px'
			...
			'./components/src/components/Molecules/OverlayResult/OverlayResult.tsx'(O, i, e) {
				'use strict';
				var r = e('../../node_modules/@emotion/react/jsx-runtime/dist/emotion-react-jsx-runtime.browser.esm.js'),
											...
											!G &&
												(0, r.Y)('div', {
													...
													children: (0, r.Y)('a', {
														...
														children: Y
															? (0, r.Y)(n._, { ...R.image })
															: (0, r.Y)(I.Q, { ...R.overlayBadge, controller: L, children: (0, r.Y)(n._, { ...R.image }) }),
													}),
												}),
											(0, r.FD)('div', {
															...
															children: [
																...
																!re && P.variants?.selections.length
																	? (0, r.Y)('div', {
																			...
																			children: P.variants?.selections.map((w) => (0, r.Y)(E.m, { ...R.variantSelection, selection: w })),
																	  })
																	: null,
																!oe &&
																	(0, r.Y)('div', {
			...
			'./components/src/utilities/snapify.ts'(O, i, e) {
				...
				class I {
					static recommendation(f) {
						const v = f.id;
						if (p[v]) return p[v];
						const E = (p[v] = W({ client: _, controller: f }));
						return (
							E.on('afterStore', async ({ controller: b }, M) => {
					...
					static autocomplete(f) {
						const v = f.id;
						if (p[v]) return p[v];
						const E = (p[v] = K({ client: _, controller: f }));
						return (
							E.on('afterStore', async ({ controller: b }, M) => {
								b.log.debug('controller', b), b.log.debug('store', b.store.toJSON()), await M();
							}),
					...
					static search(f) {
						const v = f.id;
						if (p[v]) return p[v];
						const E = (p[v] = j({ client: _, controller: f }));
						return (
							E.on('afterStore', async ({ controller: b }, M) => {
								b.log.debug('controller', b), b.log.debug('store', b.store.toJSON()), await M();
							}),
				...
				var r = function (l, a) {
					...
					(l.prototype.toName = function (d) {
						...
						if (d?.closest) {
							...
							for (var p in o) {
								var _ = ((T = C), (m = n[p]), Math.pow(T.r - m.r, 2) + Math.pow(T.g - m.g, 2) + Math.pow(T.b - m.b, 2));
								_ < c && ((c = _), (B = p));
							}
							return B;
						}
...
//# sourceMappingURL=components-Molecules-OverlayResult-OverlayResult-stories.9457f57b.iframe.bundle.js.map
