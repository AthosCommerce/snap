						...
						return (
							Object.keys(k).forEach((S) => {
								const B = k[S].action || 'merge',
									{ state: G } = k[S],
									F = k[S].ignoreParameters || [],
									q = k[S].useGlobalIgnoreParameters ?? !0 ? c.concat(F) : F,
									ee = Object.keys(r.state).filter((W) => !q.includes(W)).length == 0;
								G && ee && ye.includes(B) && (I = I[B](S, G));
							}),
							I
						);
					};
...
The error above happened in the following targeter in the Snap Config`;
				class We {
					constructor(u, k) {
						(this.mode = z.$.production),
							...
							(this.createController = async (c, r, I, S, B, G) => {
								...
								return (
									(this._controllerPromises[r.id] = new Promise((F) =>
										this._createController(c, r, I, S, B, async (g) => {
							...
							(this._createController = async (c, r, I, S, B, G) => {
								let F;
								switch (c) {
									...
									try {
										...
										if (
											...
											const {
												...
												q = { href: window.location.href, filename: r, stack: B, message: G, colno: I, lineno: S, errortimestamp: F };
							...
							Object.values(z.$).includes('production') && (this.mode = 'production'),
								...
								((c?.params?.query && 'dev' in c.params.query) || K.U.get(ve)) &&
									(c?.params.query?.dev == 'false' || c?.params.query?.dev == '0'
										? (K.U.unset(ve, I), (this.mode = z.$.production))
										: (K.U.set(ve, '1', 'Lax', 0, I), (this.mode = z.$.development)));
														...
														r?.targeters?.forEach((g, q) => {
															...
															const ee = new ne.b(
																...
																async (W, se, ue) => {
																	...
																	G(), F({ controller: me, ...W }, se, ue), me.addTargeter(ee);
																}
															);
														});
												} catch (B) {
													this.logger.error(`Failed to instantiate ${c} controller at index ${I}.`, B);
												}
											});
										});
										break;
				...
				const Pe = (0, $.PA)((p) => {
					...
					const S = k.library.getComponent(x.type, x.component),
						...
						F = (B && B[x.theme.name])?.theme;
					if (!I && !F && !k.settings?.editMode) {
				...
				class m extends We {
					constructor(u) {
						...
						const { editor: c } = x,
							...
							I = K.U.get(Ze),
							S = !!((r?.params?.query && de in r.params.query) || I === 'ui' || c?.mode === 'ui'),
							B = !!I || S || c?.mode === 'headless';
				...
				function $e(p, u) {
					return Object.keys(p.recommendation || {})
						...
						.reduce((k, x) => {
							const c = x;
							return (
								Object.keys(p.recommendation[c] || {}).forEach((r, I) => {
									const S = `recommendation/${c}`,
										B = { ...p.recommendation[c][r], index: I, type: S },
										G = {
											component: async () => {
												const F = [];
												switch (c) {
												...
												return await Promise.all(F), Pe;
											},
											props: { templatesStore: u },
											onTarget: function (F, g, q, ee) {
				...
				function Q(p, u, k) {
					...
					const I = e()(p.plugins?.custom || {}, c?.plugins?.custom || {});
					return (
				...
				var Ie = t('./components/src/themes/base/base.ts'),
					...
					Fe = t('./components/src/themes/snappy/snappy.ts'),
				...
				Ct(Te, 'base', Ie.E), Ct(Te, 'bocachica', Me.S), Ct(Te, 'pike', Je.pike), Ct(Te, 'snapnco', ke.N), Ct(Te, 'snappy', Fe.m);
				const Bt = ['primary', 'secondary', 'accent'],
					...
					wt = (0, $.PA)(({ templateStore: p, children: u, themeName: k, colorOverrides: x }) => {
						...
						return (0, o.Y)(st.Mz, {
							snap: Te,
							children: (0, o.Y)(Ce.NP, { theme: S, children: (0, o.Y)(Ft.p3, { path: 'storybook', children: u }) }),
						});
					}),
					...
					ct = [
						(p, u) => {
							...
							const I = ({ theme: B, children: G }) => (0, o.Y)(Ht, { theme: B, colorOverrides: r, children: G });
							return (0, A.gW)({
								themes: {
			...
			'./components/src/themes/bocachica/bocachica.ts'(U, C, t) {
				...
				const Zt = ({}) => (0, o.AH)({ '.ss__autocomplete__button--see-more': { margin: '10px' } }),
					...
					It = { ...pe.tablet, ...jt.tablet, ...et.tablet, ...Tt.tablet },
			...
			'./components/src/themes/pike/pike.ts'(U, C, t) {
				...
				const j = 5,
									...
									const n = s?.theme?.variables,
										...
										Ue = (0, o.AH)({
											'.ss__facet-palette-options__option': {
												...
												'.ss__facet-palette-options__option__value__count': { fontSize: '10px', color: Fe },
											},
										}),
										Ye = (0, o.AH)([
											Ue,
					...
					Ft = 'ss__grid__option',
					Te = `&.${Ft}--dark, &:has(.${Ft}__inner--grey), &:has(.${Ft}__inner--gray)`,
					Bt = '&:has(.ss__image)',
					vt = '&[style*="url"]',
			...
			'./components/src/themes/snapnco/snapnco.ts'(U, C, t) {
				...
				const lt = ({}) => (0, o.AH)({}),
					...
					It = { ...re.desktop, ...je.desktop, ...ct.desktop, ...ot.desktop },
					Kt = {
						name: 'snapnco',
						variables: {
							breakpoints: { mobile: 767, tablet: 991, desktop: 1299 },
			...
			'./src/Templates/Stores/TemplateStore.ts'(U, C, t) {
				...
				class e {
					...
					get theme() {
						...
						const Ie = [this.base.globalStyle, this.style].filter(Boolean);
						return (
							Ie.length > 0 &&
								(Q.globalStyle = ({ name: Me, variables: Je }) => Object.assign({}, ...Ie.map((ke) => ke({ name: Me, variables: Je })))),
							(Q.name = this.name),
							Q
						);
					}
				...
				class Ze {
					constructor(d) {
						...
						const $e = [],
							...
							Q = new Ee();
						$e.push(Q.promise);
						const Ie = [te, Y, this.library.import.theme[Ae.extends]()];
						Promise.all(Ie).then(() => {
							const Me = this.library.themes[Ae.extends],
								Je = Ae.overrides || {},
								ke = Ae.variables || {},
								Fe = this.library.locales.currencies[this.currency] || {},
...
//# sourceMappingURL=main.ee03cdf4.iframe.bundle.js.map
