		...
		[7853],
		{
			'./components/src/components/Molecules/Carousel/Carousel.tsx'(Be, N, e) {
				...
				const Q = ({ vertical: f, theme: y }) =>
						(0, k.AH)({
							...
							'.swiper-pagination-bullet-active': { background: y?.variables?.colors?.primary || 'inherit' },
							...
							'.swiper-pagination-bullet': {
								...
								cursor: 'pointer',
								margin: '0 4px',
								'&.swiper-pagination-bullet-active': { opacity: '0.8', background: y?.variables?.colors?.primary || '#000' },
							},
							...
							'.swiper-scrollbar': {
								...
								'&:empty': { display: 'none' },
								'.swiper-scrollbar-drag': { position: 'relative', height: '100%', backgroundColor: y?.variables?.colors?.primary || '#000' },
							},
						}),
					X = {
					...
					be = (0, $.PA)((f) => {
						...
						let d = (0, Z.v6)('carousel', y, Ae, f),
							i;
						if (
							!(f.theme?.type == 'templates' || y?.type == 'templates') &&
							d.breakpoints &&
						...
						let B = d.pagination,
							U = d.navigation,
							O = d.scrollbar;
						const he = { icon: { internalClassName: 'ss__carousel__icon', ...(0, L.s)({ disableStyles: D }), theme: d.theme, treePath: we } },
							Pe = Array.isArray(V) ? [v.Vx, v.dK, v.Ze, v.Jq].concat(V) : [v.Vx, v.dK, v.Ze, v.Jq],
							Se = Pe.filter((o, l) => Pe.indexOf(o) === l),
							fe = (0, C.li)(null),
							ve = (0, C.li)(null),
							G = (0, C.li)(null),
							Re = (0, oe.Z)(d, Q);
			...
			'./components/src/components/Templates/RecommendationBundle/RecommendationBundle.tsx'(Be, N, e) {
											...
											children: [
												x
													? (0, s.Y)('div', {
															...
															children: (0, s.Y)(Q.I, { ...S.icon, ...(typeof x == 'string' ? { icon: x } : x) }),
													  })
													: null,
													...
													children: [
														p.msrp && p.msrp !== p.price
															? (0, s.FD)('label', {
																	...
																	children: ['Was ', (0, s.Y)(f.g, { ...S.subtotalStrike, lineThrough: !0, value: p.msrp })],
															  })
															: null,
														(0, s.Y)('label', {
															className: `${n}__wrapper__cta__subtotal__price`,
															children: (0, s.Y)(f.g, { ...S.subtotalPrice, value: p.price }),
														}),
													],
												}),
											],
				...
				var Me = e('./components/src/hooks/useIntersection.tsx'),
					ye = e('./components/src/utilities/componentNameToClassName.ts');
				const Ae = ({
						vertical: i,
						separatorIcon: m,
						carousel: w,
					...
					d = (0, $.PA)((i) => {
						...
						r.alias && (c = `ss__${(0, ye.b)(r.alias)}`);
						let u;
						if (!(i.theme?.type == 'templates' || m?.type == 'templates') && ((u = (0, ne.X)(r.breakpoints)), u && Object.keys(u).length)) {
						...
						if (!(D && D.length) && !a.store?.results?.length) return a.log.error('<RecommendationBundle> Component has no results to render!'), null;
						const R = D ? D.filter((t) => t.bundleSeed == !0).pop() : a.store?.results?.filter((t) => t.bundleSeed == !0).pop(),
						...
						const es = (0, oe.Z)({ ...r, carousel: { ...Ce, slidesPerView: ce }, hasSeed: $e, carouselEnabled: De }, Ae),
							Ve = typeof b == 'number' ? b : De ? ce : g.length;
						(0, E.vJ)(() => {
							I.reset(),
								Ve &&
									g?.forEach((t, _) => {
						...
						if (De) {
							...
							if (!(i.theme?.type == 'templates' || m?.type == 'templates'))
					...
					j = (0, k.PA)((L) => {
						...
						return (
							...
							(0, s.Y)('div', {
								...
								onClick: (y) => {
									X.click && ne?.track.product.click(y, pe);
								},
								ref: ie,
								...f,
								children: Y,
