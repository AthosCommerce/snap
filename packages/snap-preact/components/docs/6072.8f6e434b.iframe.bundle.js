		...
		[935, 6072],
		{
			'./src/Instantiators/RecommendationInstantiator.tsx'(V, A, t) {
				...
				var j = t('../../node_modules/@emotion/react/jsx-runtime/dist/emotion-react-jsx-runtime.browser.esm.js'),
					...
					y = t('./src/create/createRecommendationController.ts');
				const i = 'production';
				class u {
					constructor(r, e, h) {
						if (
						...
						this.config.mode &&
							...
							(this.quickviewManager = e?.quickviewManager);
						const f = {};
								...
								if (c.profiles && c.profiles.length) {
									...
									a.forEach((m, T) => {
										if (m.selector) {
											...
											let E;
											new R.b([O], async (d, n, b, p) => {
												if (s.isConnected === !1) {
													n && p?.releaseTargets([n]);
													return;
												}
												if (d.profile?.profile || d.profile?.tag) {
													...
													if ((c.custom && (P.custom = M()(P.custom, c.custom)), !E)) E = D(this, e || {}, n, P, f, s, k, d, p);
													else {
														const I = await E;
									...
									const { profile: a, products: l, product: C, seed: m, filters: T, blockedItems: O, options: E, shopper: d, shopperId: n } = c,
										b = [].concat(l || C || m || []),
										p = [d, d?.id, n, n?.id].filter((H) => H && typeof H == 'string').pop(),
										k = {
											tag: a,
											...$({ products: b.length ? b : void 0, cart: c.cart && X(c.cart), shopper: p, filters: T, blockedItems: O, profile: E }),
										},
					...
					cleanupStaleControllers() {
						Object.keys(this.controller).forEach((r) => {
							...
							Object.values(e.targeters).some((g) =>
								g.getTargetedElems().some((_) => (_.isConnected !== !1 && _.getAttribute('ss-controller-id')) === r)
							) ||
								(Object.keys(e.targeters).forEach((g) => e.targeters[g].destroy()),
								(e.targeters = {}),
				...
				async function D(o, r, e, h, f, g, _, s, c) {
					...
					const d = { id: `recommend_${m}_${f[m] - 1}`, ...E },
						n = (0, y.default)(
							{ url: o.config.url, controller: d, context: h, mode: o.config.mode },
							{ client: o.client, tracker: o.tracker, quickviewManager: o.quickviewManager }
						);
					e && e.setAttribute('ss-controller-id', n.id),
						o.uses.forEach((p) => n.use(p)),
						o.plugins.forEach((p) => n.plugin(p.func, ...p.args)),
						o.middleware.forEach((p) => n.on(p.event, ...p.func)),
...
${g?.outerHTML}`);
						...
						return e && (await W(o, r, n, e, g, s)), n;
					}
				}
				async function W(o, r, e, h, f, g) {
...
${f?.outerHTML}`);
					...
					if (
						...
						o.config.components[s]?.onTarget?.(g, f, h, e),
...
${f?.outerHTML}`);
					...
					setTimeout(() => {
						(0, v.XX)((0, j.Y)(a, { controller: e, snap: r?.snap, ...c }), h);
					});
				}
				function X(o) {
			...
			'./src/create/createRecommendationController.ts'(V, A, t) {
				...
				const y = (i, u) => {
					...
					return (
						...
						new j.c(
							i.controller,
							{
								client: u?.client || new v.K(i.client.globals, i.client.config),
				...
				t.dn(y), t.d(A, ['default', 0, y]);
			},
		},
	]);
})();

//# sourceMappingURL=6072.8f6e434b.iframe.bundle.js.map
