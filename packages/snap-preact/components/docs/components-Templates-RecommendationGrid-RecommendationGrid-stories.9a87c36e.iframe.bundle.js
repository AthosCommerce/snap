			...
			'./components/src/components/Molecules/Result/Result.tsx'(v, i, e) {
				...
				const C = () =>
					...
					N = (0, n.PA)((q) => {
						...
						if (!me) return B;
						const T = L?.display?.mappings.core || L?.mappings?.core,
							[de, ue] = (0, l.J0)(!1),
							W = {
								variantSelection: {
						...
						let _e = T?.name;
						M.truncateTitle && (_e = y.x(T?.name || '', M.truncateTitle.limit, M.truncateTitle.append));
						const ge = (0, I.Z)(M, C),
							he = { addToCartButtonText: { value: de ? le : J }, quickviewButtonText: { attributes: { 'aria-label': 'Quick View' } } },
							Ee = f()(he, M.lang || {}),
							pe = (0, b.u)(Ee, { result: L, controller: k }, { activeBreakpoint: F?.activeBreakpoint }),
							fe = !!(T?.msrp && T?.price && T?.price < T?.msrp),
											...
											!oe &&
												(0, s.FD)('div', {
													...
													children: [
														...
														!ce && k?.quickviewManager && (0, s.Y)(U.$, { ...W.quickviewButton, ...pe.quickviewButtonText.attributes }),
													],
												}),
											(0, s.FD)('div', {
												...
												children: [
													...
													!se && ve && T.price && T.price > 0
														? (0, s.Y)('div', {
																className: 'ss__result__details__pricing',
																children: fe
																	? (0, s.FD)(s.FK, {
																			children: [
																				(0, s.Y)(c.g, { ...W.price, value: T.msrp, lineThrough: !0, name: 'msrp' }),
																				'\xA0',
																				(0, s.Y)(c.g, { ...W.price, value: T.price, name: 'price' }),
																			],
																	  })
																	: (0, s.Y)(c.g, { ...W.price, value: T.price }),
														  })
														: null,
													...
													!ae &&
														(0, s.Y)('div', {
															...
															children: (0, s.Y)(U.$, { ...W.button, ...pe.addToCartButtonText.all }),
														}),
												],
											}),
										],
					...
					E = (0, l.PA)((C) => {
						...
						if (!(C.theme?.type == 'templates' || N?.type == 'templates')) {
							const B = (0, O.X)(_?.breakpoints || {}),
								me = a()(_?.theme || {}, B?.theme || {}, { arrayMerge: (T, de) => de });
							_ = { ..._, ...B, theme: me };
						}
						const {
								disableStyles: M,
							...
							{ ComponentOverride: re, shouldWaitForNamedOverride: ae } = (0, b.x)(k, z ? V : void 0),
							H = z ? re : Y,
				...
				const d = { ResultTracker: () => (0, l.AH)({}) },
					u = (0, n.PA)((g) => {
						const P = (0, r.u)(),
							...
							{ children: I, result: y, track: w, controller: O, className: A, internalClassName: j, disableStyles: U, style: p } = m,
							f = { ...h, ...w },
							{ ref: b, inViewport: R } = (0, c.Q)();
						R && f.impression && O?.track.product.impression(y);
						const E = {};
						return (
							U ? p && (E.css = [p]) : (E.css = [d.ResultTracker(), p]),
							(0, s.Y)('div', {
			...
			'./components/src/utilities/snapify.ts'(v, i, e) {
				...
				class O {
					static recommendation(f) {
						const b = f.id;
						if (y[b]) return y[b];
						const R = (y[b] = j({ client: w, controller: f }));
						return (
					...
					static autocomplete(f) {
						const b = f.id;
						if (y[b]) return y[b];
						const R = (y[b] = U({ client: w, controller: f }));
						return (
							R.on('afterStore', async ({ controller: E }, C) => {
								E.log.debug('controller', E), E.log.debug('store', E.store.toJSON()), await C();
							}),
					...
					static search(f) {
						const b = f.id;
						if (y[b]) return y[b];
						const R = (y[b] = A({ client: w, controller: f }));
						return (
							R.on('afterStore', async ({ controller: E }, C) => {
								E.log.debug('controller', E), E.log.debug('store', E.store.toJSON()), await C();
							}),
				...
				var s = function (l, n) {
					...
					(l.prototype.toName = function (c) {
						...
						if (c?.closest) {
							...
							for (var y in o) {
								var w = ((d = P), (u = a[y]), Math.pow(d.r - u.r, 2) + Math.pow(d.g - u.g, 2) + Math.pow(d.b - u.b, 2));
								w < m && ((m = w), (h = y));
							}
							return h;
						}
...
//# sourceMappingURL=components-Templates-RecommendationGrid-RecommendationGrid-stories.9a87c36e.iframe.bundle.js.map
