...
(() => {
	(self.webpackChunk_athoscommerce_snap_preact = self.webpackChunk_athoscommerce_snap_preact || []).push([
		[2240],
		{
			'../../node_modules/@athoscommerce/snap-controller/dist/esm/Autocomplete/AutocompleteController.js'(z, O, u) {
				u.d(O, { Z: () => K });
				var l = u('../../node_modules/deepmerge/dist/cjs.js'),
					P = u.n(l),
					b = u('../../node_modules/@athoscommerce/snap-store-mobx/dist/esm/types.js'),
					H = u('../../node_modules/@athoscommerce/snap-toolbox/dist/esm/StorageStore/StorageStore.js'),
					N = u('../../node_modules/@athoscommerce/snap-controller/dist/esm/Abstract/AbstractController.js'),
					D = u('../../node_modules/@athoscommerce/snap-controller/dist/esm/utils/getParams.js'),
					F = u('../../node_modules/@athoscommerce/snap-controller/dist/esm/types.js'),
					q = u('../../node_modules/@athoscommerce/snap-controller/dist/esm/utils/isClickWithinProductLink.js'),
					R = u('../../node_modules/@athoscommerce/snap-controller/dist/esm/utils/isClickWithinBannerLink.js');
				const S = 'ss-autocomplete-input',
					y = 200,
					B = 13,
					T = 27,
					L = 'fallbackQuery',
				...
				class K extends N.r {
					constructor(o, { client: t, store: n, urlManager: a, eventManager: d, profiler: g, logger: v, tracker: w }, Q) {
						super(o, { client: t, store: n, urlManager: a, eventManager: d, profiler: g, logger: v, tracker: w }, Q),
								...
								input: {
									enterKey: async (e) => {
										if (e.keyCode == B) {
											...
											for (e.preventDefault(); this.store.loading; ) await k(y);
									...
									escKey: (e) => {
										e.keyCode == T && (e.target.blur(), this.setFocused());
									},
									focus: (e) => {
										e.stopPropagation(),
											setTimeout(() => {
									...
									formSubmit: async (e) => {
										const s = e.target,
											i = s.querySelector(`input[${S}]`);
										for (e.preventDefault(); this.store.loading; ) await k(y);
										this.store.search.correctedQuery && I(s, L, this.store.search.correctedQuery.string), await k(y + 1);
										try {
							...
							(this.search = async () => {
								try {
									...
									const p = this.store.results[0]?.responseId,
										A = p && p === h;
									if (A) {
										const m = Object.keys(this.events[h].product || {}).filter((x) => this.events[h].product?.[x]?.impression);
										this.events[h] = { product: m.reduce((x, W) => ((x[W] = { impression: !0 }), x), {}), banner: this.events[h].banner };
									...
									if (e)
										if (e.err && e.fetchDetails) {
											switch (e.fetchDetails.status) {
												...
												default: {
													this.store.error = { type: b.B.ERROR, message: e.err.message };
													break;
												}
											}
											this.log.error(this.store.error), this.handleError(e.err, e.fetchDetails);
										} else
											(this.store.error = { type: b.B.ERROR, message: `Something went wrong... - ${e}` }), this.log.error(e), this.handleError(e);
								} finally {
					...
					get params() {
						const o = this.urlManager.state,
							t = P()({ ...(0, D.j)(o) }, this.config.globals || {}),
						...
						if (
							...
							w.length && ((t.personalization = t.personalization || {}), (t.personalization.lastViewed = w.join(','))),
					...
					unbind() {
						document.querySelectorAll(`input[${S}]`)?.forEach((t) => {
			...
			'../../node_modules/@athoscommerce/snap-store-mobx/dist/esm/Autocomplete/AutocompleteStore.js'(z, O, u) {
				u.d(O, { Y: () => _ });
				var l = u('../../node_modules/mobx/dist/mobx.esm.js'),
					...
					b = u('../../node_modules/@athoscommerce/snap-store-mobx/dist/esm/Abstract/AbstractStore.js'),
					H = u('../../node_modules/@athoscommerce/snap-store-mobx/dist/esm/Search/Stores/SearchHistoryStore.js'),
					N = u('../../node_modules/@athoscommerce/snap-store-mobx/dist/esm/Search/Stores/SearchMerchandisingStore.js'),
					...
					q = u('../../node_modules/@athoscommerce/snap-store-mobx/dist/esm/Search/Stores/SearchPaginationStore.js'),
				...
				class B extends Array {
					...
					constructor(r) {
						...
						a.map((g, v) =>
							d.push(
								new T({ ...r, data: { term: { active: v === 0, value: g }, terms: d }, functions: r.functions, state: r.state, type: 'suggested' })
							)
						),
							super(...d);
					}
				...
				class L extends Array {
					...
					constructor(r) {
						const { data: o } = r || {},
							...
							n = [];
						t?.map((a) => {
							n.push(new T({ ...r, data: { term: { active: !1, value: a }, terms: n }, functions: r.functions, state: r.state, type: 'historical' }));
						}),
							super(...n);
					}
				}
				...
				class K {
					constructor(r) {
						const { services: o, data: t } = r || {},
							...
							d = {};
						n?.query && ((this.query = new I(o.urlManager, n.query)), (d.query = l.sH)),
							a?.correctedQuery && ((this.correctedQuery = new I(o.urlManager, a.correctedQuery)), (d.correctedQuery = l.sH)),
							n?.subject && ((this.subject = new I(o.urlManager, n.subject)), (d.subject = l.sH)),
				...
				var k = u('../../node_modules/@athoscommerce/snap-store-mobx/dist/esm/Search/Stores/SearchFacetStore.js');
				class M extends Array {
					...
					constructor(r) {
						const { services: o, state: t } = r || {},
							n = new k.pC({ ...r, services: { ...o, urlManager: o.urlManager.remove('filter') } });
						n.forEach((a) => {
							a.values?.forEach((d) => {
								d.preview = () => {
									n.map((g) => {
				...
				class _ extends b.K {
					constructor(r, o) {
						...
						(this.services = o),
							(this.state = new S({ services: this.services })),
					...
					initHistory() {
						...
						if (r && this.config.settings?.history?.enabled !== !1) {
							const o = new H.E({ services: this.services, config: { id: this.config.id, globals: this.config.globals } });
							this.history = new L({
					...
					update(r) {
						...
						(this.meta = new j.l({ data: { meta: o } })),
							...
							(this.merchandising = new N.W({ data: { search: t } })),
							...
							((this.results.length === 0 && !this.trending.filter((n) => n.active).length) || this.terms?.filter((n) => n.active).length) &&
								this.resetTrending(),
							(this.pagination = new q.a3({ services: this.services, data: { search: t, meta: this.meta.data } })),
