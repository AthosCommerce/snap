				...
				const W = ({ horizontal: d }) =>
					...
					E = (0, a.PA)((d) => {
						...
						const U = (y, M) => {
							...
							N = {},
							z = R()(N, S.lang || {}),
							re = (0, I.u)(z, { options: J, selectedOptions: P });
			...
			'./components/src/utilities/snapify.ts'(K, b, e) {
				...
				class I {
					static recommendation(d) {
						const T = d.id;
						if (i[T]) return i[T];
						const A = (i[T] = Z({ client: j, controller: d }));
						return (
					...
					static autocomplete(d) {
						const T = d.id;
						if (i[T]) return i[T];
						const A = (i[T] = W({ client: j, controller: d }));
						return (
							A.on('afterStore', async ({ controller: C }, S) => {
								C.log.debug('controller', C), C.log.debug('store', C.store.toJSON()), await S();
							}),
					...
					static search(d) {
						const T = d.id;
						if (i[T]) return i[T];
						const A = (i[T] = X({ client: j, controller: d }));
						return (
							A.on('afterStore', async ({ controller: C }, S) => {
								C.log.debug('controller', C), C.log.debug('store', C.store.toJSON()), await S();
							}),
						...
						$ = t.default.useCallback(
							function (o) {
								var n = V(),
									...
									p = Q(l),
									O = te(p),
									P = [].concat(g.slice(0, f), [O], g.slice(f + 1));
								u ? u(P) : S(P);
							},
							[V, Q, te, g]
						),
									...
									return {
										...
										getTickProps: function (l) {
											var p = l === void 0 ? {} : l,
												...
												F = D === void 0 ? {} : D,
												U = x(p, ['key', 'style']);
											return c({ key: P, style: c({ position: 'absolute', width: 0, left: H(n) + '%', transform: 'translateX(-50%)' }, F) }, U);
										},
									};
								});
							},
									...
									return {
										...
										getSegmentProps: function (l) {
											var p = l === void 0 ? {} : l,
												...
												N = H(o[f - 1] ? o[f - 1] : _),
												z = H(n) - N;
											return c({ key: P, style: c({ position: 'absolute', left: N + '%', width: z + '%' }, F) }, U);
										},
									};
								});
							},
...
//# sourceMappingURL=components-Organisms-FacetsHorizontal-FacetsHorizontal-stories.e54140d8.iframe.bundle.js.map
