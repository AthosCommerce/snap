				...
				const w = {
						...
						parameters: {
							docs: {
								page: () =>
									(0, t.FD)('div', {
										children: [(0, t.Y)(c.oz, { options: { overrides: { code: O.Z } }, children: B }), (0, t.Y)(c.uY, { story: c.h1 })],
									}),
							},
						},
				...
				const U = ({ horizontal: d }) =>
					...
					b = (0, r.PA)((d) => {
						...
						const V = (y, S) => {
							...
							H = {},
							Y = j()(H, T.lang || {}),
							ae = (0, k.u)(Y, { options: q, selectedOptions: C }, { activeBreakpoint: M?.activeBreakpoint });
				...
				const v = ({}) => (0, c.AH)({}),
					g = (0, D.PA)((i) => {
						const A = (0, j.u)(),
							...
							{ controller: U, toggleSideBarButton: b, disableStyles: d, className: M, internalClassName: R, treePath: P, layout: T } = F,
			...
			'./components/src/utilities/snapify.ts'(N, h, e) {
				...
				class k {
					static recommendation(d) {
						const M = d.id;
						if (i[M]) return i[M];
						const R = (i[M] = F({ client: A, controller: d }));
						return (
							R.on('afterStore', async ({ controller: P }, T) => {
					...
					static autocomplete(d) {
						const M = d.id;
						if (i[M]) return i[M];
						const R = (i[M] = U({ client: A, controller: d }));
						return (
							R.on('afterStore', async ({ controller: P }, T) => {
								P.log.debug('controller', P), P.log.debug('store', P.store.toJSON()), await T();
							}),
					...
					static search(d) {
						const M = d.id;
						if (i[M]) return i[M];
						const R = (i[M] = $({ client: A, controller: d }));
						return (
							R.on('afterStore', async ({ controller: P }, T) => {
								P.log.debug('controller', P), P.log.debug('store', P.store.toJSON()), await T();
							}),
				...
				function U(b) {
					...
					return new O.Z(b.controller, {
						client: new D.K(b.client.globals, b.client.config),
						store: new B.Y(b.controller, { urlManager: d }),
						urlManager: d,
						eventManager: new u.E(),
						profiler: new E.U(),
						logger: new v.V(),
			...
			'./components/src/utilities/storybook.tsx'(N, h, e) {
				...
				const O = 'prism-block',
					x = (D) => {
						...
						return (
							(0, c.vJ)(() => {
								B.current && D.className?.includes('lang-') && !D.className?.includes(O) && window?.Prism?.highlightElement(B.current);
							}, [D.className, D.children, B]),
							(0, t.Y)('code', { ...D, ref: B, 'data-prismjs-copy': 'Copy' })
						);
					};
				e.d(h, ['Z', 0, x]);
			},
			...
			'../../node_modules/react-ranger/dist/react-ranger.mjs'(N, h, e) {
				...
				function w(r) {
					var n = r.interpolator,
						...
						P = R[0],
						T = R[1],
						K = B({ activeHandleIndex: d, onChange: $, onDrag: F, values: v, tempValues: P }),
						J = t.default.useRef(),
						...
						Z = t.default.useCallback(
							function (o) {
								var s = K(),
									...
									p = X(l),
									I = ee(p),
									C = [].concat(v.slice(0, _), [I], v.slice(_ + 1));
								m ? m(C) : T(C);
							},
							[K, X, ee, v]
						),
									...
									return {
										...
										getHandleProps: function (m) {
											var l = m === void 0 ? {} : m,
												...
												Y = l.style,
												ae = Y === void 0 ? {} : Y,
												y = O(l, ['key', 'ref', 'innerRef', 'onKeyDown', 'onMouseDown', 'onTouchStart', 'style']);
											return c(
												{
													key: I,
...
//# sourceMappingURL=components-Organisms-Toolbar-Toolbar-stories.659f06c3.iframe.bundle.js.map
