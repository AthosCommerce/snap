				...
				const _ = { ResultTracker: () => (0, l.AH)({}) },
					f = (0, o.PA)((g) => {
						const b = (0, n.u)(),
							...
							{ children: M, result: h, track: C, controller: E, className: D, internalClassName: x, disableStyles: B, style: u } = d,
							m = { ...O, ...C },
							{ ref: v, inViewport: R } = (0, i.Q)();
						R && m.impression && E?.track.product.impression(h);
						const p = {};
						return (
							...
							(0, s.Y)('div', {
								...
								onClick: (k) => {
									m.click && E?.track.product.click(k, h);
								},
								ref: v,
								...p,
								children: M,
			...
			'./components/src/utilities/snapify.ts'(P, c, e) {
				...
				class E {
					static recommendation(m) {
						const v = m.id;
						if (h[v]) return h[v];
						const R = (h[v] = x({ client: C, controller: m }));
						return (
					...
					static autocomplete(m) {
						const v = m.id;
						if (h[v]) return h[v];
						const R = (h[v] = B({ client: C, controller: m }));
						return (
							R.on('afterStore', async ({ controller: p }, k) => {
								p.log.debug('controller', p), p.log.debug('store', p.store.toJSON()), await k();
							}),
					...
					static search(m) {
						const v = m.id;
						if (h[v]) return h[v];
						const R = (h[v] = D({ client: C, controller: m }));
						return (
							R.on('afterStore', async ({ controller: p }, k) => {
								p.log.debug('controller', p), p.log.debug('store', p.store.toJSON()), await k();
							}),
				...
				function D(u) {
					const m = new _.V(new g.E({ settings: { coreType: 'query', corePrefix: u.controller.id } }), f.X);
				...
				function x(u) {
					const m = new _.V(new g.E(), f.X).detach(!0);
				...
				function B(u) {
					const m = new _.V(new g.E(), f.X).detach();
					return new o.Z(u.controller, {
						client: new t.K(u.client.globals, u.client.config),
						store: new n.Y(u.controller, { urlManager: m }),
						urlManager: m,
						eventManager: new b.E(),
						profiler: new d.U(),
						logger: new O.V(),
...
//# sourceMappingURL=components-Trackers-ResultTracker-ResultTracker-stories.d21ecb57.iframe.bundle.js.map
