	...
	(self.webpackChunk_athoscommerce_snap_preact = self.webpackChunk_athoscommerce_snap_preact || []).push([
		[594],
		{
			'./components/src/components/Molecules/Grid/Grid.tsx'(ts, w, s) {
				...
				(0, F.X$)([rs.A]);
				const is = ({ gapSize: L, columns: d, theme: S, disableOverflowAction: j }) =>
						(0, I.AH)({
							'.ss__grid__options': {
								display: 'flex',
								flexFlow: 'row wrap',
								...
								'.ss__grid__option': {
									...
									backgroundSize: `calc(100% / ${d} - ${2 * Math.round((d + 2) / 2)}px)`,
					...
					_s = (0, _.PA)((L) => {
						...
						const R = (t, c) => {
								if (H) {
									let h;
									b.find((p) => p.value === c.value)
										? ((h = [...b]),
										  h.splice(
												h.findIndex((p) => p.value === c.value),
												1
										  ))
										: (h = [...b, c]),
										a && a(t, c, h),
							...
							B = N && T ? T * N : D.length,
							v = Math.max(0, D.length - (B - (M ? 1 : 0))),
							[i, gs] = (0, u.J0)(!!v),
							us = () => {
								const t = $ ? i : !0,
									c = { showMoreText: { value: M ? `+ ${v}` : 'Show More' }, showLessText: { value: M ? `- ${v}` : 'Show Less' } },
									h = m()(c, x.lang || {}),
									p = (0, z.u)(h, { limited: i, remainder: v }, { activeBreakpoint: d?.activeBreakpoint });
								return t && v > 0 && D.length !== B
									? (0, o.Y)('div', {
				...
				const _s = ({ theme: d }) =>
						(0, y.AH)({
							'.ss__swatches__slideshow__swatch': {
								...
								justifyContent: 'center',
								alignItems: 'center',
								border: `1px solid ${d?.variables?.colors?.primary || '#333'}`,
								aspectRatio: '1/1',
								flexDirection: 'column',
								...
								'&.ss__swatches__slideshow__swatch--selected': { border: `2px solid ${d?.variables?.colors?.primary || '#333'}` },
					...
					L = (0, os.PA)((d) => {
						...
						let a = (0, C.v6)('swatches', S, V, d);
						const X = a.breakpoints || (a.type == 'slideshow' ? x : {}),
							A = (0, P.X)(X);
						if (A && Object.keys(A).length) {
							const e = ls()(a?.theme || {}, A?.theme || {}, { arrayMerge: (f, R) => R });
							a = { ...a, ...A, theme: e };
						}
						const {
								onSelect: H,
			...
			'./components/src/hooks/useDeepCompareEffect.tsx'(ts, w, s) {
				...
				function u(r) {
					const l = (0, o.li)(r),
						m = (0, o.li)(0);
					return (0, I.j)(r, l.current) || ((l.current = r), (m.current += 1)), (0, o.Kr)(() => l.current, [m.current]);
				}
				function y(r, l) {
					return (0, o.vJ)(r, [u(l)]);
				}
