...
The grouped block uses a `script[type="athos/recommendations"]` tag containing a `profiles` array. Each entry in `profiles` specifies a `tag` (or `profile`) and a `selector` targeting where the component should render.
...
The legacy block uses individual `script[type="athos/recommend"]` tags with a `profile` attribute. Each script tag results in its own controller.
