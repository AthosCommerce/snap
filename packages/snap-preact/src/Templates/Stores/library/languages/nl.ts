...
export const nl: LangComponents = {
	...
	searchHeader: {
		...
		expandedSearchText: {
			value: (data) => {
				return `We konden geen exacte match vinden voor "<span className="ss__search-header__results-query">${data?.search?.query?.string}</span>", maar hier is iets soortgelijks:`;
			},
		},
	},
	noResults: {
