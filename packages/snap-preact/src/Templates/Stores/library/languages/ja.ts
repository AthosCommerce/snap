...
export const ja: LangComponents = {
	...
	searchHeader: {
		...
		expandedSearchText: {
			value: (data) => {
				return `"<span className="ss__search-header__results-query">${data?.search?.query?.string}</span>"に完全に一致する結果は見つかりませんでしたが、類似の結果は次のとおりです：`;
			},
		},
	},
	noResults: {
		...
		suggestionsList: {
			value:
				'<ul className="ss__no-results__suggestions__list"><li className="ss__no-results__suggestions__list__option">スペルミスがないか確認してください。</li><li className="ss__no-results__suggestions__list__option">余分なキーワード（例："商品"）を削除してください。</li><li className="ss__no-results__suggestions__list__option">別の言葉でお探しの内容を表現してみてください。</li></ul>',
		},
		// contactsTitleText: {
