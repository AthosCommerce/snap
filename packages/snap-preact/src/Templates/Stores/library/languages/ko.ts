...
export const ko: LangComponents = {
	...
	searchHeader: {
		...
		expandedSearchText: {
			value: (data) => {
				return `"<span className="ss__search-header__results-query">${data?.search?.query?.string}</span>"에 대한 정확히 일치하는 결과를 찾을 수 없었지만, 비슷한 결과는 다음과 같습니다:`;
			},
		},
	},
	noResults: {
		...
		suggestionsList: {
			value:
				'<ul className="ss__no-results__suggestions__list"><li className="ss__no-results__suggestions__list__option">철자가 틀리지 않았는지 확인하세요.</li><li className="ss__no-results__suggestions__list__option">불필요한 키워드(예: "제품")를 제거하세요.</li><li className="ss__no-results__suggestions__list__option">찾으시는 내용을 다른 단어로 표현해 보세요.</li></ul>',
		},
		// contactsTitleText: {
