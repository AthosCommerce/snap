...
export const pt: LangComponents = {
	...
	searchHeader: {
		...
		expandedSearchText: {
			value: (data) => {
				return `Não conseguimos encontrar uma correspondência exata para "<span className="ss__search-header__results-query">${data?.search?.query?.string}</span>", mas aqui está algo parecido:`;
			},
		},
	},
	noResults: {
