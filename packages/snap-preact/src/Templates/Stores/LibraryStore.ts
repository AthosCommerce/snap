...
export type CurrencyCodes = typeof currencyCodes[number];
...
export class LibraryStore {
	...
	import: LibraryImports = {
		...
		// all currencies live in a single module (lazily loaded once) — the per-code importers keep the same shape as language/theme importers
		currency: currencyCodes.reduce((currencyImports, code) => {
			currencyImports[code] = async () => {
				return this.locales.currencies[code] || (this.locales.currencies[code] = (await import('./library/currencies/currencies')).currencies[code]);
			};
