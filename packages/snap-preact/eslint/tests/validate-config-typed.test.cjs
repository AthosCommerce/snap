// typescript-estree infers "single-run" mode when process.env.CI === 'true' and switches
// to a one-shot program lifecycle, which breaks under jest's many-lints-per-process usage
// (every verify after the first gets a fresh program with no tsconfig). Pin it off before
// the parser loads, or this suite silently loses type information on CI runners.
